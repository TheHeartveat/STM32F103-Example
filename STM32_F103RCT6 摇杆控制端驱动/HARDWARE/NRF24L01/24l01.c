...
u8 NRF24L01_Write_Reg(u8 reg,u8 value)
{
	u8 status=0;	
  	...
  	status =SPI2_ReadWriteByte(reg);//发送寄存器号  （选择写入的寄存器（地址））
...
u8 NRF24L01_Read_Reg(u8 reg)
{
	u8 reg_val=0;	    //存储寄存器的值
 	NRF24L01_CSN = 0;          //使能SPI传输		
  	SPI2_ReadWriteByte(reg);   //发送寄存器号     	（选择读取的寄存器（地址））
  	reg_val=SPI2_ReadWriteByte(0XFF);//读取寄存器内容   （0xFF：空指令，可随便定义，对读取没有影响）
  	NRF24L01_CSN = 1;          //禁止SPI传输		    
...
u8 NRF24L01_Read_Buf(u8 reg,u8 *pBuf,u8 len)
{
	u8 status,u8_ctr;	       //status：状态值
  	NRF24L01_CSN = 0;           //使能SPI传输
  	status=SPI2_ReadWriteByte(reg);//发送寄存器值(位置),并读取状态值   	   （选择读取的寄存器（地址））
 	for(u8_ctr=0;u8_ctr<len;u8_ctr++)pBuf[u8_ctr]=SPI2_ReadWriteByte(0XFF);//读出数据				（0xFF：空指令，可随便定义，对读取没有影响）
...
u8 NRF24L01_TxPacket(u8 *txbuf,u8 len)
{
	...
	NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,len);//写数据到TX BUF  32个字节		把txbuf指针指向的数组的各个成员的数值写到发送缓冲区（地址：WR_TX_PLOAD）内，TX_PLOAD_WIDTH个字节（32个）
...
{
	u8 sta=0; 
	SPI2_SetSpeed(SPI_BaudRatePrescaler_2); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）  设置SPI通信传输的速度 
	...
	{
		NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf,len);//读取数据    RD_RX_PLOAD：读取的寄存器的地址 rxbuf：存入的数组的首地址  RX_PLOAD_WIDTH：接收的字节数   可用sizeof计算，例如：sizeof（a）,结果是变量a占用的字节数
